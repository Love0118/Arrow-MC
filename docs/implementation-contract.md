...
2026-09-06 사용자 지시에 따라 **구현 선택과 애매한 로직은 Pumpkin의 Rust 구현부터 확인**한다.
완성도·성능·구조를 평가해 참고하고, 누락·오류·26.3 버전 차이는 기존 Vanilla 디컴파일본과 실행 결과로 보완한다.
목표는 독립 Rust 서버이며 JVM 연산·HotSpot 내부·마지막 부동소수 비트의 재현은 목표로 삼지 않는다.
seed 알고리즘·난수 소비 순서·packet 인과관계처럼 실제 게임 결과를 결정하는 규칙은 유지한다.
작은 수치 차이는 측정·기록하고, 게임 동작에 의미 있는 영향이 확인된 경우 필요한 범위에서 해결한다.
...
- NBT 작업에 구현이 과도하게 집중되지 않도록 실제 서버 접속, chunk/tick owner, 병렬 처리 경로를 함께 진행한다.
  item-data gate를 무관한 status/network/runtime의 대기 조건으로 확대하지 않는다.
  검수된 기반 batch는 닫고 다음 NBT 범위는 구체적인 소비자 요구에서 재개한다.
- 구현 에이전트에도 실제 파일 소유 범위를 배정한다. 리뷰어만 배치하고 구현을 중단하지 않는다.
- 리뷰어는 구현과 별도로 Pumpkin의 해당 구현과 부족한 부분을 확인하고, 필요한 Vanilla 근거·실행 대조·회귀 사례를 검증한다.
  리뷰 통과를 구현자의 자기보고로 대신하지 않으며, 게임 기능 진행을 JVM 구현 세부 재현으로 대체하지 않는다.
