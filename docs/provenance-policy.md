...
| `src/server/chunk_sender.rs` | PlayerChunkSender와 fastutil/Guava 공개 동작을 실제 실행해 quota·동률 규칙을 확인한 후 bounded pending/selection/delivery를 독립 설계 | 원문 method body·주석을 복사하지 않고 별도 Rust 자료구조 작성, 추가 dependency 없음 | 호출자가 실제 readiness와 packet body를 제공. Play encoder·TCP 연결은 미완료 |
